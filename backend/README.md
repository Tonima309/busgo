...
This project now includes a starter Laravel API for React apps using token authentication via Laravel Sanctum.

### Included API Endpoints

- `GET /api/v1/health` - Health check endpoint.
- `POST /api/v1/auth/register` - Register a new user and return access token.
- `POST /api/v1/auth/login` - Login and return access token.
- `GET /api/v1/auth/me` - Get authenticated user (`auth:sanctum` required).
- `POST /api/v1/auth/logout` - Revoke current token (`auth:sanctum` required).
...
Default dev origins already included:
